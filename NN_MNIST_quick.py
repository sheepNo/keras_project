...
from keras.layers import Input, Dense, Dropout, Activation, Conv2D, Flatten, MaxPooling2D, AveragePooling2D
...
model.add(Conv2D(28, kernel_size=(4,4), strides=(2,2), activation='relu', input_shape=(28,28, 1)))
model.add(Dropout(0.25))
# add dropouts
model.add(Conv2D(14, kernel_size=(8,8), strides=(4,4), activation='relu'))
model.add(Dropout(0.25))
model.add(AveragePooling2D(pool_size=(2,1))) # 2,1 is pretty good
model.add(Flatten())
...
model.fit_generator(datagen.flow(data, labels, batch_size=200), epochs=8, validation_data=(test, testlabels))
